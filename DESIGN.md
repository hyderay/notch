...
- An `NSStatusItem` provides settings, hook installation, and quit. The persistent **Hide overlay in full screen** preference is enabled by default; active-Space and frontmost-application changes update overlay visibility without discarding session state.

Hover is detected by **polling the cursor position**, not with an `NSTrackingArea`. The obvious approach fails here: expanding resizes the panel, which makes AppKit call `updateTrackingAreas`, tear the area down, and emit a spurious exit/enter pair — so the overlay oscillates between compact and expanded several times a second. Polling at 12.5 Hz (only while the overlay is visible) sidesteps that, and the hit region is hysteretic: entering requires the cursor to be on the compact strip, leaving requires it to exit the entire panel. Measured over a 14-second synthetic hover, this produces exactly one enter and one exit.
