...
- An `NSStatusItem` provides settings, hook installation, and quit. The persistent **Hide overlay in full screen** preference is enabled by default; active-Space and frontmost-application changes update overlay visibility without discarding session state. Detection reads the focused window's `AXFullScreen` attribute because native full-screen windows on notched Macs leave the camera band outside their Quartz bounds. Three delayed checks span the Space transition animation, with Quartz coverage retained as a fallback when the attribute is unavailable.
